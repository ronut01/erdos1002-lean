...
\hypersetup{
  ...
  pdfsubject={Erdos Problem 1002; continued fractions and Cauchy limits},
...
\date{August 25, 2026}
\subjclass[2020]{Primary 11K38; Secondary 11A55, 37A50, 60F05}
\keywords{continued fractions, discrepancy sums, Cauchy law, stable limits,
Gauss map, Erd\H{o}s Problem 1002}
...
A Lean~4 formalization of this proof has been completed and independently
verified.
...
$\sum_{a\ge1}a^{-2+t_0}(1+(\log a)^k)$.  For the Gauss-specific
Ruelle--Mayer framework underlying this perturbation argument, see
\cite{Vallee97}.  Analytic perturbation of the simple leading eigenvalue
\cite[Chap.~VII]{Kato95} therefore gives, after decreasing $t_0>0$ if necessary,
\[
 \begin{aligned}
...
\[
 ...
 \le \exp(C_Ku).
\]
Thus the local counts have uniformly bounded exponential moments.
Passing from the $C_c^1$ test functions above to indicators is justified by
inner and outer $C_c^1$ approximation on compact $\Lambda$-continuity sets;
the truncation windows used below are finite unions of intervals.
On finitely many disjoint compact continuity sets, the preceding calculation
...
mixing probability-preserving endomorphism is mixing.  Concretely, every
finite two-sided digit cylinder is a fixed iterate of a one-sided cylinder,
so by invariance a correlation of two such cylinders is a one-sided Gauss
cylinder correlation with separation $m-O(1)$; Lemma~\ref{lem:gauss-estimates}(i)
then factors it as $m\to\infty$.  Density of finite cylinders gives the
natural-extension mixing statement.
...
The author thanks Ibrahim Mian and Shayaan Siddique of Millennium Research for
...
referee opinion; Mian and Siddique are not authors of this manuscript.  The
completed formalization and verification materials are available in the
companion repository \cite{KwonMianSiddiqueLean26}.
...
\bibitem{KwonMianSiddiqueLean26}
S.~Kwon, I.~Mian, and S.~Siddique,
\emph{Lean~4 formalization of A Cauchy Limit for a Sawtooth Sum},
GitHub repository, 2026,
\url{https://github.com/ronut01/erdos1002-lean}.
...
\bibitem{Vallee97}
B.~Vall\'ee,
\emph{Op\'erateurs de Ruelle--Mayer g\'en\'eralis\'es et analyse en moyenne
des algorithmes d'Euclide et de Gauss},
Acta Arith. 81 (1997), no.~2, 101--144.
